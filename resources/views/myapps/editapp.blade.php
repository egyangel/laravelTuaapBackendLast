    ...
    <div class="row">
      <div class="col-12"> 
          <div class="box-header text-center">
            <h3 class="box-title">Edit New App</h3>
          </div>
          <!-- /.box-header --> 
 
              @include('errors.error')

              {!! Form::open(['url' => ["/myapp" , $myapp->id ], 'class' => 'form-group' , 'files' => true]) !!}
              {{ method_field('PUT') }}
                          <div class="form-group{{ $errors->has('appname') ? ' has-error' : '' }}">
                              <label for="appname" class=" control-label"> App Name</label>
                              <div class=" ">
                                  <input id="appname" type="text" class="form-control" name="appname" value="{{ $myapp->appname }}" required autofocus>
